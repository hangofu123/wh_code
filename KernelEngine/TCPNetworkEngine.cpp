		...
		while (m_wRecvSize>=sizeof(TCP_Head))
		{
			...
			if (Command.wMainCmdID == MDM_KN_COMMAND)
			{
				...
				case SUB_KN_DETECT_SOCKET:	//网络检测
					{
						//LogTxtOut(TEXT("CTCPNetworkItem  网络检测"));
						break;
					}
				default:
					{
	...
	case ASYNCHRONISM_DETECT_SOCKET:	//检测连接
		{
			...
			for (INT_PTR i=0;i<m_TempNetworkItemArray.GetCount();i++)
			{
				...
				if (pTCPNetworkItem->IsAllowSendData()==true)
				{
					...
					case DANGER_QUOTIETY+1:
					case DANGER_QUOTIETY+2:
					case DANGER_QUOTIETY+3:
						{

							pTCPNetworkItem->m_wSurvivalTime--;
							DetectSocket.dwSendTickCount = GetTickCount();
							pTCPNetworkItem->SendData(&DetectSocket, sizeof(DetectSocket),MDM_KN_COMMAND,SUB_KN_DETECT_SOCKET,pTCPNetworkItem->GetRountID());
							break;
						}
					default:				//默认处理
						{ 
...
bool CTCPNetworkEngine::WebAttestation()
{
	...
	//		//提示信息
	//		CTraceService:: TraceString(TEXT("您的服务器组件没有得到合法授权，请联系“疯石游戏” http://www.gamexx.net"),TraceLevel_Exception);
