...
VOID CGameFrameViewGDI::DrawUserClock(CDC * pDC, INT nXPos, INT nYPos, WORD wUserClock)
{
	...
	if ((wUserClock>0)&&(wUserClock<100))
	{
		...
		{
			//加载资源
			CWHPngImage ImageClockItem;
			CWHPngImage ImageClockBack;
...
//绘画数字
VOID CGameFrameViewGDI::DrawNumberString(CDC * pDC, CWHPngImage &PngNumber, LONG lNumber, INT nXPos, INT nYPos,BYTE cbAlign)
{
	//加载资源
...
VOID CGameFrameViewGDI::DrawMatchInfo(CDC * pDC)
{
	...
	{
		//定义变量
		CWHPngImage ImageWaitTip;
		CWHPngImage ImageNumberOrange;
		CWHPngImage ImageNumberGreen;
		CWHPngImage ImageLine;
