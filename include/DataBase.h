...
public:
	...
	//接口查询
	virtual void *  QueryInterface(const IID & Guid, DWORD dwQueryVer);
	virtual bool GetBurthenInfo(tagBurthenInfo & BurthenInfo) { return true; };
	//配置模块
	virtual bool SetDataBaseEngineSink(IUnknownEx * pIUnknownEx[], WORD wSinkCount);
	virtual bool DeferDataBaseRequest(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize) { return true; };
	//服务接口
public:
	//启动服务
