...
public:
	...
	//外部资源
public:
	CWHPngImage						m_ImageEnter;						//进入位图
	CBitImage						m_ImageChair;						//椅子位图
	CBitImage						m_ImageTable;						//桌子位图
	CBitImage						m_ImageGround;						//背景位图
	CWHPngImage						m_ImageCustomUser;					//自定位图

	//内部资源
public:
	CWHPngImage						m_ImageReady;						//准备位图
	CWHPngImage						m_ImageLocker;						//桌锁位图
	CWHPngImage						m_ImageNumberNB;					//号码位图
	CWHPngImage						m_ImageNumberBK;					//号码背景
...
public:
	...
	CWHPngImage						m_ImageNumberGreen;					//绿色数字
	CWHPngImage						m_ImageNumberYellow;				//黄色数字
	CWHPngImage						m_ImageLine;						//图片资源
...
protected:
	...
	//绘画数字
	VOID DrawNumberString(CDC * pDC, CWHPngImage &PngNumber, LONG lNumber, INT nXPos, INT nYPos,BYTE cbAlign=TA_LEFT);
